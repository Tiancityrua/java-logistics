...
//            Cell8.setCellValue((String) list.get(i-1).get(""));
    ...
    @ResponseBody
    @RequestMapping(value = "/business/invoice/creditors",method = RequestMethod.POST)
    public void credit_invoice(@RequestBody Map map,HttpServletResponse response){
        List<Map> resultlist=service.se_creditors(map);
        HSSFWorkbook wb=new HSSFWorkbook();
        HSSFSheet sheet=wb.createSheet("sheet1");
        HSSFRow Row=sheet.createRow(0);
        HSSFCell Cell1=Row.createCell(0);
